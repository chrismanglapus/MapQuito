document.addEventListener("DOMContentLoaded", function () {
  ...
  function calculateThresholds(weeklyData, selectedYear) {
    ...
    const thresholds = Array.from({ length: maxWeeks }, (_, weekIndex) => {
      const weekCases = historicalYears.map((year) => {
        const weekData = weeklyData[year]?.find(
  ...
  function updateChart(selectedYear, weeklyData) {
    ...
    function getMorbidityWeeks(year) {
      if (cachedData && cachedData.weekly_data[year]) {
        const availableWeeks = cachedData.weekly_data[year].map(
          (entry) => entry.week
        );
    ...
    const cases = weeklyData[selectedYear]
      .slice(0, morbidityWeeks.length)
      .map((entry) => parseFloat(entry.total_cases) || 0.0);
    const thresholds = calculateThresholds(weeklyData, selectedYear).slice(
      0,
      morbidityWeeks.length
    );

    const previousYear = selectedYear - 1;
    const prevYearMaxWeeks = isLeapYear(previousYear) ? 53 : 52;
    const prevYearCases = weeklyData[previousYear]
      ? weeklyData[previousYear]
          .slice(0, prevYearMaxWeeks)
          .map((entry) => parseFloat(entry.total_cases) || 0.0)
      : [];
        ...
        plugins: {
          ...
          tooltip: {
            ...
            external: function (context) {
              ...
              const casesData = context.tooltip.dataPoints.find((dp) =>
                dp.dataset.label.includes("Cases in")
              );
              if (!casesData) {
                tooltipEl.style.opacity = "0";
                return;
              ...
              tooltipEl.innerHTML = `
                                <div style="font-size: 18px; font-weight: bold; padding-bottom:;">WEEK ${
                                  weekIndex + 1
                                }</div>
              ...
              document.getElementById("alert-threshold").innerText =
                thresholds[weekIndex].alertThreshold;
              document.getElementById("epidemic-threshold").innerText =
                thresholds[weekIndex].epidemicThreshold;

              const canvas = context.chart.canvas;
              const canvasRect = canvas.getBoundingClientRect();

              const tooltipX = context.tooltip.caretX + canvasRect.left;
              const tooltipY = context.tooltip.caretY + canvasRect.top;

              const tooltipWidth = tooltipEl.offsetWidth;
              const tooltipHeight = tooltipEl.offsetHeight;

              // Detect if sidebar is open or closed by checking a class on sidebar or body
              const sidebar = document.getElementById("sidebar");
              let sidebarOffset = 0;
              if (sidebar && !sidebar.classList.contains("close")) {
                sidebarOffset = 250; // sidebar is open
              }
              // else sidebarOffset = 0 if closed

              const centerX = canvasRect.left + canvasRect.width / 2;

              let left;
              if (tooltipX < centerX) {
                // Tooltip on right side of cursor + offset for sidebar
                left = tooltipX + 15 - sidebarOffset;
                if (left < sidebarOffset + 10) left = sidebarOffset + 10; // don't go behind sidebar
              } else {
                // Tooltip on left side of cursor - width - offset for sidebar
                left = tooltipX - tooltipWidth - 15 - sidebarOffset;
                if (left < sidebarOffset + 10) left = sidebarOffset + 10;
              }

              let top = tooltipY - tooltipHeight / 2;
              if (top < 10) top = 10;
  ...
  function updateLegend(weekIndex, thresholds) {
    ...
    if (weekIndex === null) {
      ...
      document.getElementById("alert-threshold").textContent =
        thresholds[weekIndex].alertThreshold;
      document.getElementById("epidemic-threshold").textContent =
        thresholds[weekIndex].epidemicThreshold;
